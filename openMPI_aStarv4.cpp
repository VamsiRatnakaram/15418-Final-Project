...
void hashedSendFunction(pPair pairToSend, int dim_x, int nproc, int procID, std::priority_queue<pPair, vector<pPair>, compare> *openList, cell cellToSend, cell *cellDetails) {
	...
	if (indexToSend == procID) {
        if (cellDetails[(x*dim_x + y)/nproc].f == INT_MAX || (cellDetails[(x*dim_x + y)/nproc].f > pairToSend.first)) {
            openList->push(pairToSend);
            cellDetails[(x*dim_x + y)/nproc] = cellToSend;
        }
	}
	else {
...
double aStarSearch(int *map, Pair src, Pair dest, int dim_x, int dim_y, int procID, int nproc, const char* input_filename)
{
    ...
    cell *cellDetails = (cell*)calloc(dim_x*dim_y/nproc + 1, sizeof(cell));

	int i, j;

	for (i = 0; i < dim_x*dim_y/nproc + 1; i++) {
    ...
    if (procID == 0) {
        ...
        cellDetails[(i*dim_y+j)/nproc].f = 0.0;
        cellDetails[(i*dim_y+j)/nproc].g = 0.0;
        cellDetails[(i*dim_y+j)/nproc].h = 0.0;
        cellDetails[(i*dim_y+j)/nproc].parent_i = i;
        cellDetails[(i*dim_y+j)/nproc].parent_j = j;
    }
	...
	while (conditionFlag) {
        for (int node = 0; node < nproc; node++)
        {
            ...
            {	
                ...
                while (flag_data && flag_cell)
                {
                    ...
                    if (indexToSend == procID) {
                        if (cellDetails[(x*dim_y + y)/nproc].f == INT_MAX || (cellDetails[(x*dim_y + y)/nproc].f > tmpPair.first)) {
                            openList.push(tmpPair);
                            cellDetails[(x*dim_y+y)/nproc] = tmpCell;
                        }
                    }
                    MPI_Iprobe(node, TAG_DATA, MPI_COMM_WORLD, &flag_data, MPI_STATUS_IGNORE);
                    MPI_Iprobe(node, TAG_CELL, MPI_COMM_WORLD, &flag_cell, MPI_STATUS_IGNORE);
        ...
        if (openList.size() != 0) {
            ...
            // printf("this is proc %d, im working on node %d, %d\n", procID, i, j);
            ...
            for (int z = 0; z < 4; z++) {
                ...
                if (isValid(x, y, dim_x, dim_y) == true) {
                    ...
                    if (isDestination(x, y, dest) == true) {
                        ...
                        int fnew=cellDetails[(i*dim_y+j)/nproc].g+1.0;
                        localDestCost=fnew;
                        ...
                        gNew = cellDetails[(i*dim_y+j)/nproc].g + 1.0;
    ...
    if (procID == 0) {
        ...
        while(tmpPair.first != src.first || tmpPair.second != src.second) {
            ...
            if (ownerNode != procID) {
                ...
                tmpPair = make_pair(cellDetails[(tmpPair.first*dim_y + tmpPair.second)/nproc].parent_i, cellDetails[(tmpPair.first*dim_y + tmpPair.second)/nproc].parent_j);
            }
        }
        traceDone=true;
        Path.push(tmpPair);
    ...
    while (!traceDone) {
		...
		if (flag) {
            ...
            parentPair = make_pair(cellDetails[(tmpPair.first*dim_y + tmpPair.second)/nproc].parent_i, cellDetails[(tmpPair.first*dim_y + tmpPair.second)/nproc].parent_j);
